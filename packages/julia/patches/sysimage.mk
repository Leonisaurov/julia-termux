...
default: sysimg-$(JULIA_BUILD_MODE)
...
RELDATADIR := $(call rel_path,$(JULIAHOME)/base,$(build_datarootdir))/
...
.SECONDARY: $$(build_private_libdir)/basecompiler$1-o.a $$(build_private_libdir)/basecompiler$1-bc.a $$(build_private_libdir)/sysbase$1.ji
endef

# Helper variables for bundled LLVM tools
LLVM_AR  = $(build_bindir)/../tools/llvm-ar
LLVM_LINK = $(build_bindir)/../tools/llvm-link
LLVM_LLC  = $(build_bindir)/../tools/llc

define sysimg_builder
# Step 1: Julia produces LLVM bitcode archive (.bc.a) — cheaper than ELF object emission
$$(build_private_libdir)/sysbase$1-bc.a : $$(build_private_libdir)/basecompiler$1.$$(SHLIB_EXT) $$(JULIAHOME)/VERSION $$(BASE_SRCS) $$(STDLIB_SRCS)
	@$$(call PRINT_JULIA, cd $$(JULIAHOME)/base && \
	if ! JULIA_BINDIR=$$(call cygpath_w,$$(build_bindir)) \
	     WINEPATH="$$(call cygpath_w,$$(build_bindir));$$$$WINEPATH" \
	 JULIA_NUM_THREADS=1 \
			$$(call spawn, $$(JULIA_EXECUTABLE)) -g0 $2 -C "$$(JULIA_CPU_TARGET)" $$(HEAPLIM) --output-bc $$(call cygpath_w,$$@).tmp $$(JULIA_SYSIMG_BUILD_FLAGS) \
			--startup-file=no --warn-overwrite=yes --sysimage $$(call cygpath_w,$$<) sysimg.jl --buildroot $$(RELBUILDROOT) --dataroot $$(RELDATADIR); then \
		echo '*** This error might be fixed by running `make clean`. If the error persists$$(COMMA) try `make cleanall`. ***'; \
		false; \
	fi )
...
# Step 2: Extract bitcode from archive, compile individual .bc files to .o, archive
$$(build_private_libdir)/sysbase$1-o.a : $$(build_private_libdir)/sysbase$1-bc.a
	@echo "    LLC     $$@"
	@$(JULIAHOME)/contrib/bc2obj.sh $$(call cygpath_w,$$<) $$(call cygpath_w,$$@) \
		$(LLVM_AR) $(LLVM_LINK) $(LLVM_LLC)

# Step 2b: Same pipeline for the sys (precompile) step
$$(build_private_libdir)/sys$1-o.a : $$(build_private_libdir)/sys$1-bc.a
	@echo "    LLC     $$@"
	@$(JULIAHOME)/contrib/bc2obj.sh $$(call cygpath_w,$$<) $$(call cygpath_w,$$@) \
		$(LLVM_AR) $(LLVM_LINK) $(LLVM_LLC)

# sys-bc.a only — sys-o.a is built by bc2obj from sys-bc.a (Step 2b above)
$$(build_private_libdir)/sys$1-bc.a : $$(build_private_libdir)/sysbase$1.$$(SHLIB_EXT) $$(JULIAHOME)/contrib/generate_precompile.jl
	@$$(call PRINT_JULIA, cd $$(JULIAHOME)/base && \
	if ! JULIA_BINDIR=$$(call cygpath_w,$(build_bindir)) \
	 WINEPATH="$$(call cygpath_w,$$(build_bindir));$$$$WINEPATH" \
	 ...
	 JULIA_DEPOT_PATH=':' \
	 JULIA_NUM_THREADS=1 \
			$$(call spawn, $3) $2 -C "$$(JULIA_CPU_TARGET)" $$(HEAPLIM) --output-bc $$(call cygpath_w,$$@).tmp $$(JULIA_SYSIMG_BUILD_FLAGS) \
	...
	@mv $$(call cygpath_w,$$@).tmp $$(call cygpath_w,$$@)
.SECONDARY: $$(build_private_libdir)/sys$1-o.a $$(build_private_libdir)/sys$1-bc.a
.SECONDARY: $$(build_private_libdir)/sysbase$1-o.a $$(build_private_libdir)/sysbase$1-bc.a
endef
$(eval $(call base_builder,,-O1,$(JULIA_EXECUTABLE_release)))
$(eval $(call base_builder,-debug,-O0,$(JULIA_EXECUTABLE_debug)))
$(eval $(call sysimg_builder,,-O0,$(JULIA_EXECUTABLE_release)))
