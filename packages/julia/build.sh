...
XC_HOST:=\
    ...
    if [ -f deps/llvm.mk ]; then
        ...
        if ! grep -q 'CROSS_TOOLCHAIN_FLAGS_LLVM_NATIVE' deps/llvm.mk 2>/dev/null; then
            sed -i '/LLVM_ENABLE_ZLIB=FORCE_ON/a LLVM_CMAKE += -DLLVM_USE_HOST_TOOLS=ON -DCROSS_TOOLCHAIN_FLAGS_LLVM_NATIVE="-DCMAKE_SYSTEM_NAME=Linux;-DCMAKE_C_COMPILER=gcc;-DCMAKE_CXX_COMPILER=g++;-DCMAKE_C_FLAGS=;-DCMAKE_CXX_FLAGS=;-DCMAKE_EXE_LINKER_FLAGS=;-DCMAKE_SHARED_LINKER_FLAGS=;-DCMAKE_MODULE_LINKER_FLAGS="' deps/llvm.mk 2>/dev/null || echo "Warning: H16 native LLVM tools patch failed" >&2
        fi
    fi
